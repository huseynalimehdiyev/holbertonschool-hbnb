...
## 1. Presentation Layer
This is the layer that users directly interact with. It includes: API endpoints and Services. When a user sends a request (for example, creating a user or viewing places), it first comes to this layer. The layer then forwards the request to the system through the Facade, acting as the main entry point.

## 2. Business Logic Layer
This is the “brain” of the application.This layer decides what should happen in the system. It includes:User, Place, Review, Amenity
...
## 3. Persistence Layer
This layer handles data storage.It communicates directly with the database and ensures everything is stored correctly. It includes: Repository classes (e.g., UserRepository, PlaceRepository)
...
The Facade pattern makes communication between layers much simpler. Instead of the Presentation Layer directly talking to the Business Logic Layer, everything goes through the Facade.
...
This architecture keeps the system well-structured by clearly separating responsibilities between layers. Using the Facade pattern also makes the system easier to work with by providing a simple and unified way for different parts of the application to communicate.
